...
const stringToValue = (s: string): string => {
  switch (s) {
    case "nine":
      return "9";
    case "eight":
...
const result = pipe(
  ...
  A.map((x) =>
    pipe(
      ...
      O.bind("first", () =>
        O.fromNullable(
          x.match(/\d|(one|two|three|four|five|six|seven|eight|nine)/) as
            | string[]
            | null
        )
      ),
      O.bind("last", () =>
        O.fromNullable(
         (x
            ...
            .reverse()
            .join("")
            .match(/\d|(eno|owt|eerht|ruof|evif|xis|neves|thgie|enin)/) as
            | string[]
            | null)
        )
      ),
      O.map(
        ({ first, last }) =>
          stringToValue(first[0]) +
          stringToValue(last[0].split("").reverse().join(""))
      )
    )
  ),
  (v) => {
  ...
  A.map(O.map(parseInt)),
  A.reduce(0, (a: number, b: O.Option<number>) => a + O.getOrElse(() => 0)(b))
);
